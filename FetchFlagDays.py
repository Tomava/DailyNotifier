...
def main():
    base_url = "https://intermin.fi"
    # https://intermin.fi/suomen-lippu/liputuspaivat/2021
    url = f"{base_url}/liputuspaivat-{CURRENT_YEAR}"
    respData = fetch_url(url)

    all_flag_days = {}
    table_html = re.findall(
        f'data-analytics-asset-title="Liputuspäivät vuonna {CURRENT_YEAR}(?s:.*?)</div>',
        str(respData),
    )
    flag_days_html = re.findall("<li>(.*?)</li>", str(table_html))
    print(f"Found {len(flag_days_html)} flag days. List of flag days: {flag_days_html}")
    for flag_day in flag_days_html:
        ...
        print(f"Adding flag day: {date} - {day_name} ({status})")
